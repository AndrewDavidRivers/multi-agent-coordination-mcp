#!/usr/bin/env python3
"""
Stdio MCP Server for Coordinating Multiple Parallel Agents
This server uses stdio transport for reliable Cursor integration
"""
        ...
        async def handle_call_tool(name: str, arguments: Dict[str, Any]) -> List[TextContent]:
            ...
            try:
                if name == "get_instructions":
                    return [TextContent(type="text", text=self._get_instructions())]
                    ...
                    result = self._get_next_todo_item(arguments['project_name'], arguments['agent_id'])
                    return [TextContent(type="text", text=json.dumps(result, indent=2))]
                
                elif name == "update_todo_status":
                    result = self._update_todo_status(arguments['todo_id'], arguments['status'], arguments['agent_id'])
                    return [TextContent(type="text", text=json.dumps(result, indent=2))]
...
"""
    ...
    def _create_task(self, project_name: str, name: str, description: str, order: int = 0, dependencies: List[int] = None) -> dict:
    ...
    def _create_todo_item(self, task_id: int, title: str, description: str = "", order: int = 0, dependencies: List[int] = None, files: List[str] = None) -> dict:
        if dependencies is None:
            dependencies = []
        if files is None:
            files = []
    ...
    def _get_next_todo_item(self, project_name: str, agent_id: str) -> dict:
        with get_db() as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT id FROM projects WHERE name = ?", (project_name,))
            project_row = cursor.fetchone()
    ...
    def _update_todo_status(self, todo_id: int, status: str, agent_id: str) -> dict:
    ...
    def _check_file_locks(self, files: List[str]) -> dict:
        with get_db() as conn:
            cursor = conn.cursor()
            locked_files = {}
            for file_path in files:
    ...
    def _lock_files(self, files: List[str], agent_id: str) -> dict:
        with get_db() as conn:
            cursor = conn.cursor()
            locked_by_others = []
            for file_path in files:
    ...
    def _unlock_files(self, files: List[str], agent_id: str) -> dict:
        with get_db() as conn:
            cursor = conn.cursor()
            unlocked_files = []
            not_owned = []
    ...
    def _get_project_status(self, project_name: str) -> dict:
        with get_db() as conn:
            cursor = conn.cursor()
    ...
    def _insert_todo_item(self, task_id: int, title: str, description: str = "", after_todo_id: Optional[int] = None, dependencies: List[int] = None, files: List[str] = None) -> dict:
        if dependencies is None:
            dependencies = []
        if files is None:
            files = []
